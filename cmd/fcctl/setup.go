...
func setup() {
	...
	baseDir := envOr("FCCTL_BASE", "/opt/firecrackerlacker")
	fmt.Println("==> firecrackerlacker setup")
	fmt.Printf("    base: %s\n", baseDir)
	...
	fmt.Println("\n==> Creating API key")
	db := filepath.Join(baseDir, "db/firecrackerlacker.db")
	st, err := store.New(db)
	if err != nil {
	...
	fmt.Printf("  systemctl start firecrackerlacker\n\n")
	fmt.Printf("Or manually:\n")
	fmt.Printf("  %s/bin/firecrackerlacker-server \\\n", baseDir)
	fmt.Printf("    --images-dir %s/images \\\n", baseDir)
	fmt.Printf("    --state-dir %s/state \\\n", baseDir)
	fmt.Printf("    --db %s/db/firecrackerlacker.db\n", baseDir)
}
...
[Service]
Type=simple
ExecStart=%s/bin/firecrackerlacker-server \
  --images-dir %s/images \
  --state-dir %s/state \
  --db %s/db/firecrackerlacker.db \
  --prometheus \
  --max-per-key 10 \
  --rate-limit 30
Restart=on-failure
...
`, baseDir, baseDir, baseDir, baseDir)
	...
	os.WriteFile("/etc/systemd/system/firecrackerlacker.service", []byte(service), 0644)
	run("systemctl", "daemon-reload")
	run("systemctl", "enable", "firecrackerlacker")
